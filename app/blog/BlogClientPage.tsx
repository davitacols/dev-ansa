...
export default function BlogClientPage({ searchParams }: BlogPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { posts: fetchedPosts, total: fetchedTotal } = await getBlogPosts({
        take: pageSize,
        skip,
        categorySlug,
        tagSlug,
      ...
      setActiveCategory(fetchedCategories.find((c) => c.slug === categorySlug))
      setActiveTag(fetchedTags.find((t) => t.slug === tagSlug))
    }

    fetchData()
  }, [searchParams, skip, pageSize, categorySlug, tagSlug, searchQuery])
